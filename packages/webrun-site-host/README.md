...
| Export | Purpose |
| --- | --- |
| `HostedSiteBuilder` | Fluent builder. Chain `setSiteKey` / `setFiles` / `setEndpoint` / `setServerRunner` / `setAuth` / `setErrorHandler`, then `await .build()` for a `HostedSite`. |
...
delegates to the module's `default` export. Swap the file content and
the next request sees the new module; the browser's import cache
dedupes repeated fetches of the same URL.
